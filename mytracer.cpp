...
MyTracer::MyTracer(QObject *parent) : QObject(parent)
{

}

MyTracer::MyTracer(QCustomPlot *_plot, QCPGraph *_graph, TracerType _type, QObject *parent) :
    plot(_plot),
    graph(_graph),
    type(_type),
    visible(false)
{
    if (plot)
    {
        ...
        tracer->setStyle(QCPItemTracer::tsCrosshair);//可以选择设置追踪光标的样式，这个是小十字，还有大十字，圆点等样式
