    ...
    , ui(new Ui::Widget)
{
    ...
    ui->setupUi(this);
    sleepinterventionwidget=new SleepInterventionWidget();
    contralDataWidget=new ContralDataWidget();
    commsetwidget=new CommSetWidget();
    setWindowTitle(tr("控制窗口"));//设置标题

    ui->widget_contralData->addWidget((QWidget*)contralDataWidget);
    ui->widget_SleepInterven->addWidget(sleepinterventionwidget);
    ui->widget_comm->addWidget((QWidget*)commsetwidget);

    connect(sleepinterventionwidget,SIGNAL(sendSignal(QByteArray)),this,SLOT(send_command(QByteArray)));
    connect((QWidget*)contralDataWidget,SIGNAL(sendSingal(QByteArray)),this,SLOT(send_command(QByteArray)));
    connect((QWidget*)commsetwidget,SIGNAL(dataSignal(QVector<double>)),this,SLOT(dataPlot(QVector<double>)));

    connect((QWidget*)commsetwidget,SIGNAL(fileStart(QString)),contralDataWidget->getCtrlData(),SLOT(SaveBegin(QString)));//文件转存信号与ctrldata的savebegin连接
    connect((QWidget*)commsetwidget,SIGNAL(fileEnd()),contralDataWidget->getCtrlData(),SLOT(SaveEnd()));//文件转存结束信号
...
//    qDebug()<<"ControlWnd::dataPlot"<<data;
    ...
    Stride stride=contralDataWidget->getCtrlData()->getStride();
    int FP_stride=stride.fp;
    int light_stride=stride.light;
    int snore_stride=stride.snore;
    int groacc_stride=stride.groacc;
    int max_num=contralDataWidget->getCtrlData()->getMaxNum();
    int fp_index=-1;
...
//        qDebug()<<chart_data;
        curvegroup->append(chart_data.toList());
    }
}
...
void ControlWnd::get_eegwnd(CurveGroup *eegwnd,OutPutWnd *outputwnd){
    this->curvegroup=eegwnd;
